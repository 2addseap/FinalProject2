...
HashNode* favoriteTable[HASH_SIZE] = {NULL};
...
void insert_favorite_to_table(const char* code) {
    int index = hash(code);
    HashNode* new_node = malloc(sizeof(HashNode));
    strcpy(new_node->code, code);
    new_node->next = favoriteTable[index];
    favoriteTable[index] = new_node;
}

int is_favorite_in_table(const char* code) {
    int index = hash(code);
    HashNode* current = favoriteTable[index];
    while (current) {
...
void remove_favorite_from_table(const char* code) {
    int index = hash(code);
    HashNode* current = favoriteTable[index];
    HashNode* prev = NULL;
    ...
    while (current) {
        if (strcmp(current->code, code) == 0) {
            if (prev)
                ...
                favoriteTable[index] = current->next;
...
typedef struct BSTNode {
    int index; // Index into houses[]
    float price;
    struct BSTNode* left;
    struct BSTNode* right;
} BSTNode;

BSTNode* priceTreeRoot = NULL;
...
BSTNode* insert_to_price_tree(BSTNode* root, int index, float price) {
    if (root == NULL) {
    ...
    if (price < root->price)
        root->left = insert_to_price_tree(root->left, index, price);
    else
        root->right = insert_to_price_tree(root->right, index, price);
...
void load_favorites_to_hash_table() {
...
int favorite_house(int house_index) {
    ...
    if (is_favorite_in_table(h.code)) {
    ...
    insert_favorite_to_table(h.code); // ← Insert into hash table too
...
void remove_favorite_by_index(int remove_index) {
    ...
    char line[256];
    int current = 0;
    char removed_code[10] = "";

    while (fgets(line, sizeof(line), file)) {
        if (current == remove_index) {
            sscanf(line, "%9[^,]", removed_code);  // Save the code being removed
        } else {
            fputs(line, temp);
        }
        current++;
    ...
    if (strlen(removed_code) > 0) {
        remove_favorite_from_table(removed_code);  // ← Update hash table too
    }
}
...
void display_houses_by_price(BSTNode* root) {
    if (!root) return;

    display_houses_by_price(root->left);

    int i = root->index;
    printf(YELLOW_COLOR "\nHouse: %s\n" RESET_COLOR, houses[i].name);
    printf(WHITE_COLOR "Province: " RESET_COLOR "%s\n", houses[i].province);
    printf(WHITE_COLOR "Price: " RESET_COLOR "%.2f\n", houses[i].price);
    printf(WHITE_COLOR "Rating: " RESET_COLOR "%.1f\n", houses[i].rating);
    printf(WHITE_COLOR "Bedrooms: " RESET_COLOR "%d\n", houses[i].bedrooms);

    display_houses_by_price(root->right);
}
...
while (1) {
    ...
    if (scanf("%f", &min_price) != 1 || min_price < 0) {
        printf(RED_COLOR "Invalid input. Price must be 0 or greater.\n" RESET_COLOR);
        flush_input();
        continue;
    }
    flush_input();
...
while (1) {
    ...
    if (min_rating > 0 && max_rating > 0 && min_rating > max_rating) {
        printf(RED_COLOR "\nMinimum rating cannot be greater than maximum rating. Try again.\n" RESET_COLOR);
    } else {
        break;
    }
}
...
while (1) {
    ...
    if (scanf("%d", &min_bedrooms) != 1 || min_bedrooms < 0) {
        printf(RED_COLOR "Invalid number. Must be 0 or more.\n" RESET_COLOR);
        flush_input();
        continue;
    }
    flush_input();
...
while (1) {
    ...
    if (scanf("%d", &min_beds) != 1 || min_beds < 0) {
        printf(RED_COLOR "Invalid number. Must be 0 or more.\n" RESET_COLOR);
        flush_input();
        continue;
    }
...
while (1) {
    ...
    if (scanf("%d", &min_bathrooms) != 1 || min_bathrooms < 0) {
        printf(RED_COLOR "Invalid number. Must be 0 or more.\n" RESET_COLOR);
        flush_input();
        continue;
    }
...
void customer_menu() {
    ...
    while (1) {
        ...
        printf("4. My Booking\n");
        printf("5. View Houses Sorted by Price\n");
        printf("0. Back\n");
        printf("Choose: ");
        scanf("%d", &choice);
        ...
        switch (choice) {
            ...
            case 5:
    clear_screen();
    printf(BLUE_COLOR "HOUSES SORTED BY PRICE:\n" RESET_COLOR);
    display_houses_by_price(priceTreeRoot);
    printf(YELLOW_COLOR "\nPress Enter to return..." RESET_COLOR);
    getchar(); getchar();
    break;
...
void run_system() {
    ...
    while (1) {
        ...
        switch (choice) {
            case 1:
                load_favorites_to_hash_table();
                load_houses_for_customer("Briefly_Info.csv"); 
                priceTreeRoot = NULL;
for (int i = 0; i < house_count; i++) {
    if (houses[i].is_available)
        priceTreeRoot = insert_to_price_tree(priceTreeRoot, i, houses[i].price);
}
                load_detailed_houses_from_csv("Detail.csv");
                customer_menu();
                break;
